...
function publicGame(game) {
  return {
    ...
    buzzerOpen: game.buzzerOpen, buzzedPlayer: game.buzzedPlayer, buzzedAtMs: game.buzzedAtMs,
...
function makeGame(code, config) {
  ...
  return {
    ...
    boards,
    boardIndex: 0,
    final: config.final, buzzerOpen: false, buzzerOpenedAt: null, buzzedPlayer: null, buzzedAtMs: null, activeQuestion: null, reveal: false, finalResponses: new Map(), turnPlayerId: null, answeringPlayerId: null, incorrectPlayerIds: [], clueDeadline: null, cluePausedRemaining: null, answerDeadline: null, finalDeadline: null, lastEvent: null
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('host:action', ({ code, action, payload }) => {
    ...
    if (action === 'select') {
      ...
      if (q && !q.used) {
        ...
        game.clueDeadline=null; game.cluePausedRemaining=null; game.answerDeadline=null; game.buzzerOpenedAt=null; game.buzzedPlayer = null; game.buzzedAtMs=null; game.answeringPlayerId = null; game.incorrectPlayerIds=[]; game.buzzerOpen = false; game.reveal = false
        emitGame(game); return
      }
    }
    if (action === 'buzzer') { if(game.activeQuestion && !game.reveal && !game.answeringPlayerId) { game.buzzerOpen = !!payload.open; if (payload.open) { game.buzzedPlayer = null; game.buzzedAtMs=null; game.buzzerOpenedAt=Date.now(); const remaining=game.cluePausedRemaining ?? 45000; game.clueDeadline=Date.now()+remaining; game.cluePausedRemaining=null; schedule(game,'clue',remaining,()=>revealThenReturn(game,'reveal',true)) } } }
    if (action === 'score') {
      ...
      if (p && amount > 0 && game.status === 'playing' && game.activeQuestion && !game.activeQuestion.final) {
        ...
        const remaining=game.cluePausedRemaining ?? 45000; game.clueDeadline=Date.now()+remaining; game.cluePausedRemaining=null; game.buzzerOpen=true; game.buzzerOpenedAt=Date.now(); game.buzzedAtMs=null; schedule(game,'clue',remaining,()=>revealThenReturn(game,'reveal',true))
      }
    }
    if (action === 'close') {
      clearGameTimers(game)
      if (game.activeQuestion && !game.activeQuestion.final) markUsed(game)
      game.activeQuestion = null; game.clueDeadline=null; game.cluePausedRemaining=null; game.answerDeadline=null; game.buzzerOpen = false; game.buzzerOpenedAt=null; game.buzzedPlayer = null; game.buzzedAtMs=null; game.answeringPlayerId = null; game.reveal = false
    ...
    if (action === 'nextBoard') { game.boardIndex = 1; game.activeQuestion = null; game.clueDeadline=null; game.cluePausedRemaining=null; game.buzzerOpen = false; game.buzzerOpenedAt=null; game.buzzedPlayer = null; game.buzzedAtMs=null; game.reveal = false }
  ...
  socket.on('player:buzz', ({ code, playerId }) => {
    ...
    game.buzzedAtMs=Math.max(0,Date.now()-(game.buzzerOpenedAt||Date.now())); game.lastEvent={type:'buzz',playerId,at:Date.now()}; openAnswerWindow(game, playerId)
  })
  socket.on('player:dailyWager', ({ code, playerId, wager }) => {
    ...
    active.dailyWager=value; active.dailyReady=true; game.answeringPlayerId=null; game.buzzerOpenedAt=Date.now(); game.buzzedAtMs=null; game.clueDeadline=Date.now()+45000
    emitGame(game); schedule(game,'clue',45000,()=>{player.score-=value;game.turnPlayerId=playerId;revealThenReturn(game)})
  })
